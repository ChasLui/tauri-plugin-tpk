...
/// API config type so Tauri accepts `plugins.tpk` as `null`, `{}`, or a
...
/// `plugins.tpk`, or passed programmatically.
///
...
pub fn init<R: Runtime>(
    ...
    let config: HotswapConfig = context
        ...
        .ok_or_else(|| {
            Error::Config("missing or invalid 'plugins.tpk' in tauri.conf.json".into())
        })?;

    let endpoint = config
        .endpoint
        .clone()
        .ok_or_else(|| Error::Config("'endpoint' is required in plugins.tpk config".into()))?;
...
///
/// ```rust,ignore
/// let (plugin, context) = tauri_plugin_tpk::HotswapBuilder::new("<YOUR_MINISIGN_PUBKEY>")
...
fn build_plugin<R: Runtime>(
    ...
    let plugin = Builder::<R, Value>::new("tpk")
...
mod tests {
    ...
    /// `plugins.tpk` is absent from tauri.conf.json.
    #[test]
    ...
    /// `plugins.tpk` during `Builder::run()`.
    #[test]
